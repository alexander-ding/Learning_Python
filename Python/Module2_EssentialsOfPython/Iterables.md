...
常见的可迭代物包括列表，元组，和字符串——任何你可以用for循环迭代的序列。我们也将遇到一些并非序列的集合，如词典（dictionary）和集（set）；这些也是可迭代物。也可以有可迭代物在迭代时“产生”（generate）一个成员——也就是说它甚至不需要同时在内存中存储其所有的成员。我们会在后文花一整节来讨论产生器（generator），一种特殊的迭代器，因为它们会帮助我们编写高效率的代码。
